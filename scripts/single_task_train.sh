...
for i in "${!tasks[@]}"; do
  task="${tasks[$i]}"
  docker run --gpus device=$i --rm --network host --ipc=host --user $(id -u) \
    -v /home/zheong/HN-LoRA:/user/octo \
    -v /data/lynx/zheong/octo/data:/user/octo/data \
    -v /data/lynx/zheong/octo/finetune_saves:/user/octo/finetune_saves \
    -v /data/lynx/zheong/octo/eval_results:/user/octo/eval_results \
    -it zheng/hn_lora \
    python ./scripts/finetune.py --config=scripts/configs/finetune_config.py:head_only,language_conditioned \
