...
def generate_attention_map(patch_attention, image_size=(256, 256), patch_size=(16, 16)):
    heatmap = np.zeros([patch_attention.shape[0]] + list(image_size))
    max_row = image_size[0] // patch_size[0]
    max_col = image_size[1] // patch_size[1]
    for i in range(patch_attention.shape[1]):
        row = i // max_col
        col = i % max_col
        heatmap[:, row * patch_size[0]:(row + 1) * patch_size[0], col * patch_size[1]:(col + 1) * patch_size[1]] = patch_attention[:, i][:, None, None]
    return heatmap


def combine_image_and_heatmap(images, heatmaps):
    combined_images = []
    for i in range(images.shape[0]):
        image, heatmap = images[i], heatmaps[i]
    ...
    return np.stack(combined_images)
